...
const CONVERSE_PROMPT: &str = "You write a short, natural conversation between two souls of Thrushcombe St Mary, a 1934 West-Country market town, as they fall into talk. Stay wholly in period voice and in character, each in a distinct voice true to their station and feeling. At most a brief greeting, then get to something real — the season's work, money owed, a marriage, a grievance, a piece of news, a scheme, a sly dig. They must NOT merely echo or restate each other; each line should answer, press, disagree, tease, confide or turn to a new matter, so the talk actually goes somewhere. Let their differences in rank and regard show. Vary the phrasing and do not lean on stock fillers — avoid starting lines with 'I daresay' or 'I warrant'. Write 4 to 6 short lines in all, alternating, each one or two sentences, prefixed with the speaker's name and a colon. No narration, no stage directions, no preamble.";
